...
import { checkDomain, checkAlternativeTlds, POPULAR_TLDS, TECH_TLDS, COUNTRY_TLDS, } from "./domain-checker.js";
const HELP = `
    ...
    try {
        if (values.alt || values.tech || values.country || values.tlds) {
            ...
            if (isJson) {
                ...
                console.log("\n" + "=".repeat(60));
                console.log(`Summary: ${available.length} available, ${taken.length} taken out of ${results.results.length} checked`);
            }
        }
        else {
            // Check single domain
